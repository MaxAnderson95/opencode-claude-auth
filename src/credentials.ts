...
function syncToPath(authPath: string, creds: ClaudeCredentials): void {
  ...
  auth.anthropic = {
    ...
    // OpenCode's auth schema rejects a non-integer `expires`. Credentials read
    // straight from the keychain/file bypass parseOAuthResponse's Math.trunc,
    // so floor again at the sink.
 ...
 * even under a minimal PATH (e.g. a launchd-managed OpenCode server, where PATH
 * is just /usr/bin:/bin:/usr/sbin:/sbin and a bare `claude` is not found).
 * Honors CLAUDE_CLI_PATH, then known install locations, then a shell PATH
 * lookup, and finally the bare command name.
 */
