...
// Pin the CLI path so resolveClaudeBinary short-circuits to the override
// instead of probing the host's filesystem/PATH. Keeps the spawn counts in the
// child-process stub deterministic and independent of whether the machine
// running the tests actually has `claude` installed.
process.env.CLAUDE_CLI_PATH = "/test/bin/claude"
...
async function loadCredentialsWithCountingKeychain(
  ...
  const rewritten = sourceCredentials
    ...
    // The `import type { StdioOptions }` line has no runtime effect, but it
    // must not resolve to the stub, which exports no types.
    .replace(
      'import type { StdioOptions } from "node:child_process"',
      "type StdioOptions = unknown",
    )
...
// A spawned claude CLI succeeds here; the keychain read hook is what decides
// whether it rotated the stored credential. POSIX spawns it via execFileSync
// (arg array), Windows via execSync (shell string) — both count as one spawn.
...
export function __getCliSpawnCount() {
  return execFileSyncCount + execSyncCount
}
`,
    "utf8",
  )
...
describe("credential caching", () => {
  ...
  it("refreshIfNeeded does not adopt a stale stored blob over valid in-memory credentials", async () => {
    ...
    try {
      ...
      assert.equal(result?.accessToken, "fresh-in-memory")
      assert.equal(
        childProcessModule.__getCliSpawnCount(),
        0,
...
describe("refreshViaCli command shape", () => {
  it("uses the stable haiku alias, not a dated model ID", () => {
    ...
    // POSIX path passes an arg array to execFileSync; the Windows path builds
    // a shell string. Both must use the stable alias.
    assert.match(source, /"--model",\s*"haiku"/)
    assert.match(source, /-p \. --model haiku/)
...
describe("syncAuthJson expiry normalization", () => {
